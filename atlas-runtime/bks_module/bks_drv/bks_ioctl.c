...
/* There should be a psf manager which adjust psf dynamically according to
 * kernel's profiling results. Here we randomly flip the psf for testing
 * purposes. */
static int bks_fake_psf_manager(struct file *filp, u64 va) {
    struct bks_proc_ctx *proc_ctx = filp->private_data;
    // randomly flip the following 64 psf values, i.e. 128 MB region in total
    const unsigned max_psf_len = 64;
    u64 random_bytes;
    unsigned psf_idx, psf_len;
    int i;

    if (!proc_ctx->psf) {
        printk(KERN_ERR "bks: %s: ctx->psf is NULL\n", __func__);
        goto out;
    }

    if (va > BKS_PSF_VA_BASE ||
        va <= BKS_PSF_VA_BASE - BKS_PSF_MAX_MEM_MB * 1024 * 1024)
        goto out;

    get_random_bytes(&random_bytes, sizeof(u64));

    psf_idx = (BKS_PSF_VA_BASE - va) >> BKS_PSF_CHUNK_SHIFT;
    psf_len = psf_idx + max_psf_len > BKS_PSF_MMAP_SIZE
                  ? BKS_PSF_MMAP_SIZE - psf_idx
                  : max_psf_len;

    for (i = 0; i < psf_len; ++i) {
        struct bks_psf *psf = &proc_ctx->psf[psf_idx + i];
        if ((random_bytes >> i) & 0x1) {
            if (psf->psf != 0)
                psf->psf = 0;
            else
                psf->psf = 1;
...
static int bks_fetch_object_ioctl(struct file *filp, void __user *args) {
    ...
    if (bks_ctl_flag(BKS_FAKE_PSF_MANAGER) && !ret) {
        bks_fake_psf_manager(filp, fetch_object_args.obj_page_addr);
    }
    return ret;
}
