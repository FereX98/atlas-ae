...
enum bks_ctl_flag_type {
    BKS_FAKE_PSF_MANAGER,
    BKS_DIRECT_SWAP,
	NUM_BKS_CTL_FLAGS
};

extern bool bks_ctl_flags[NUM_BKS_CTL_FLAGS];

int bks_debugfs_init(void);
void bks_debugfs_exit(void);

static inline bool bks_ctl_flag(enum bks_ctl_flag_type type)
{
	return READ_ONCE(bks_ctl_flags[type]);
}
