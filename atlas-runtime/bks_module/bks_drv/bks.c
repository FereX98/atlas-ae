...
static int bks_init(void) {
    ...
    error = bks_debugfs_init();
    if (error) {
        printk(KERN_ERR "bks: fail to init debugfs\n");
        return error;
    }
...
int bks_mmap(struct file *file, struct vm_area_struct *vma) {
    ...
    if (vma->vm_pgoff == BKS_CARD_MMAP_PGOFF) {
        ...
        ctx->psf = psf;
        if(bks_ctl_flag(BKS_DIRECT_SWAP)){
            printk(KERN_INFO "bks: enable the direct swap optimization (one-to-one mapping)\n");
            //vma->vm_mm->atlas_mm = true;
        }else{
            printk(KERN_INFO "bks: disable the direct swap optimization (one-to-one mapping)\n");
	    }
    } else {
        int i;
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Lei Chen <chenlei18s@ict.ac.cn>");
