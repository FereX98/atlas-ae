...
function run_compress {
    ...
    local_ratio=$2
    kernel_tag=$3
    echo "Running uncompress, local memory size $local_size, local memory ratio $local_ratio"
    ...
    numactl --cpubind=1 --membind=1 cgexec -g memory:snappy ${exec_dir}/main > $log_dir/uncompress.${kernel_tag}.log.${local_ratio} 2>&1
}
