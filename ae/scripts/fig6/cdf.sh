...
function run_client {
    ...
    ssh_run "cd ${client_dir}; bash run-latency.sh at 1 --print_percentiles"

    sleep 1
    scp ${client_host}:/home/osdi/caladan/logs/at-mcdcll-1.log /home/osdi/ae/results/fig6b/atlas.log
    if [ $? -ne 0 ]; then
        echo "scp workloads failed"
        exit 1
    fi
    python3 /home/osdi/ae/results/fig6b/process.py
    sleep 1
}
