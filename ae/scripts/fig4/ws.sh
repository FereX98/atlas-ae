...
function run {
    ...
    for target_mops in ${target_mops_arr[@]}; do
        ...
        do
            ...
            do
                log_name=atlas-${ratio}-raw
                executable="zipf_web-ol"
                nontemp="false"
                sudo pkill -9 zipf_web
                if [[ "$nt" == "nt" ]]; then
                    nontemp="true"
                fi
                sed "s/const bool non_temporal = .*/const bool non_temporal = ${nontemp};/g" ${app_path}/zipf_web-offered-load.cpp -i
                sed "s/memory.limit_in_bytes=.*hash/memory.limit_in_bytes=${mem} hash/g" ${app_path}/zipf_web-offered-load.cpp -i
                sed "s/constexpr double target_mops = .*/constexpr double target_mops = $target_mops;/g" ${app_path}/zipf_web-offered-load.cpp -i
                #sed "s|save_to_file(\"logs/.*-raw\"|save_to_file(\"logs/$log_name-raw\"|g" ${app_path}/zipf_web-offered-load.cpp -i
                ...
                cgexec -g memory:hash --sticky taskset -c 24-41 ${app_path}/${executable} | tee $log_file/$log_name
                if [ $? -ne 0 ]; then
                    echo "Error: run failed"
                    exit 1
                fi
                python3 /home/osdi/ae/results/fig4/ws/process.py ${ratio}
            done
        done
    done
}
