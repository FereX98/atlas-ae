...
function run_client {
    ...
    sleep 1
    scp ${client_host}:/home/osdi/caladan/logs/at-mcdcl-${local_ratio}p.log /home/osdi/ae/results/fig4/mcd-cl/atlas-${local_ratio}-raw
    if [ $? -ne 0 ]; then
        echo "scp workloads failed"
        exit 1
    fi
    python3 /home/osdi/ae/results/fig4/mcd-cl/process.py ${local_ratio}
    sleep 1
}
