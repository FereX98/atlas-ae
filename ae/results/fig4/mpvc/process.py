...
def parse_results(input_file, output_file):
    pattern = re.compile(r"total time: (\d+) us")
...
if __name__ == "__main__":
    ...
    if int(ratio) not in [13, 25, 50, 75, 100]:
        print("Invalid ratio")
        sys.exit(1)
    input_file = f"/home/osdi/ae/results/fig4/mpvc/atlas-{ratio}-raw"
    if not os.path.exists(input_file):
        print(f"Input file {input_file} does not exist")
        sys.exit(1)
    output_file = f"/home/osdi/ae/results/fig4/mpvc/atlas-{ratio}"
